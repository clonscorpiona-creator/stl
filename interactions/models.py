...
class CommentLike(models.Model):
    ...
    def _update_counts(self):
        self.comment.likes_count = CommentLike.objects.filter(comment=self.comment).count()
...
class Notification(models.Model):
    ...
    TYPE_CHOICES = [
        ...
        ('save', 'Сохранение'),
        ('mention', 'Упоминание'),
        ('new_work', 'Новая работа'),  # Новая работа в любимой категории
        ('complaint', 'Жалоба'),  # Жалоба на работу/комментарий
        ('recommendation', 'Рекомендация'),  # Рекомендация работы
    ]
